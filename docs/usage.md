...
- the developer needs to provide changelog fragments for the changeset and may want to review the unreleased changelog
...
This repository uses the tool to keep the changelog, so you can see it in action (look at the `changelog` folder).
...
Repositories using this tool are expected to leverage the `pr-has-fragment` command to validate the presence of a Changelog Fragment in a Pull Request. The command can be configured to validate PRs without a Changelog Fragment but labelled with specific labels.
...
As a maintainer, you are responsible for ensuring **changelog fragments** are present in Pull Requests.
You may also be interested in viewing the changelog for unreleased changes.
...
Given a PR number, this command checks for the presence of an added Changelog Fragment.

For further details, look at command usage: `elastic-agent-changelog-tool pr-has-fragment --help`
...
The side effect is that the changelog will include all entries from latest stable release up to the current pre-release, but in the end, this correctly reflects the content of the provided pre-release.
...
> [!NOTE]
> We do not remove fragments, as they will be needed for the stable release version changelog.
...
1. From the root folder of the repository, run:
    ...
    * `repo` is the repository's name containing the issues and PRs. The default value is `elastic-agent`.
    ...
    >NOTE: If no changes are included in the release (there are no changelog fragment files), the changelog file will be generated with an empty array of entries.
...
1. From the root folder of the repository, run:
    ...
    The files that are generated depend on the specified `file_type`. The destination directory depends on the `rendered_changelog_destination` defined in the repo's `config.changelog.yaml`. If no `rendered_changelog_destination` is specified, it will be added to the `changelog` directory.
    ...
    * `asciidoc`: One file will be created, `<rendered_changelog_destination>/<version>.asciidoc`, and you will need to integrate the generated content into the changelog.
